...
$(document).ready( function() {
    $('.header__burger').click(function (event) {
        $('.header__burger,.menu-item').toggleClass('active');
        $('body').toggleClass('lock');
    });
    $('.menu__link').click(function (event) {
        $('.header__burger,.menu-item').removeClass('active');
...
tabsButtons.forEach(btn => {
  ...
  btn.addEventListener('click', () => {
    ...
    const prevActiveItem = document.querySelector('.home-catalog__tabs._active');
...
function init () {
    ...
    var myMap = new ymaps.Map('map', {
            ...
            center: [53.960318, 27.031773], // Минск
            zoom: 16
        });
